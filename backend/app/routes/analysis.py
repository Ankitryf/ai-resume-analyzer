...
async def analyze_resume(
    ...
    try:
        ...
        exp_explanation = analysis_data.get("experience_explanation", "")

        _, matched_areas, exp_gaps, assessment = semantic_analyzer.analyze_experience(
            resume_text, jobDescription
        )
        ...
        recruiter_verdict = analysis_data.get("recruiter_verdict", "Unavailable")

        readiness = analysis_data.get("readiness_level", "Unavailable")

        improvement_priorities = analysis_data.get("recommendations", [])
        ...
        relevance_score = (matched_count / max(1, matched_count + missing_count)) * 100
...
async def get_analysis(
    ...
    analysis = (
        ...
        .filter(
            AnalysisResult.id == analysisId, AnalysisResult.user_id == current_user.id
        )
        .first()
    )
    ...
    response = {
        ...
        "aiEnabled": bool(
            analysis.skill_gaps
            or analysis.experience_analyses
            or analysis.inferred_skills
        ),
    }
...
async def download_report(
    ...
    score_color = (
        ...
        else ("orange" if analysis.ats_score >= 50 else "red")
    )
    ...
    story = [
        ...
        Paragraph(
            f"<font color='grey'>Generated {created} &nbsp;|&nbsp; {filename}</font>",
            body,
        ),
        Spacer(1, 14),
        *section("ATS Score"),
        Paragraph(
            f"<font color='{score_color}' size='18'><b>{analysis.ats_score:.1f}%</b></font>",
            body,
        ),
        Paragraph(
            f"Format Score: {analysis.format_score:.1f}% &nbsp;|&nbsp; Experience Match: {analysis.experience_match_score:.1f}%",
            body,
        ),
        ...
        *[
            Paragraph(
                f"• <b>{sg.skill_name}</b> [{sg.priority}] — {sg.why_it_matters}",
                bullet_style,
            )
            for sg in analysis.skill_gaps
        ],
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=analysis-{analysisId}.pdf"
        },
    )
